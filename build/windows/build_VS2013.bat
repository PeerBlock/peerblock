...
REM  PeerBlock copyright (C) 2009-2014 PeerBlock, LLC
...
REM Compile PeerBlock with MSVC 2013
...
IF DEFINED PB_CERT (
  ...
  FOR %%F IN (
    "bin13\Win32\Release" "bin13\Win32\Release_(Vista)" "bin13\x64\Release" "bin13\x64\Release_(Vista)"
  ) DO (
    PUSHD %%F
...
"%InnoSetupPath%\iscc.exe" /SStandard="cmd /c "..\..\..\bin\windows\sign_driver.cmd" $f "^
 /Q /O"..\..\..\distribution" "setup.iss" /DVS2013build
...
:SubMSVC
TITLE Compiling PeerBlock with MSVC 2013 - %~1^|%~2...
"MSBuild.exe" PeerBlock.sln /t:%BUILDTYPE% /p:Configuration=%1^
...
COPY /Y /V "bin13\%1\%~2\peerblock.exe" "temp_zip\"
COPY /Y /V "bin13\%1\%~2\pbfilter.sys"  "temp_zip\"
