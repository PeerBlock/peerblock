											 ...
											 VOID* packet, const FWPS_FILTER0* filter, UINT64 flowContext, FWPS_CLASSIFY_OUT0* classifyOut)
{
	...
	if(!action) {
		...
		pgn.source.addr6.sin6_family = AF_INET6;
		pgn.source.addr6.sin6_addr = *localAddr;
		pgn.source.addr6.sin6_port = (USHORT)NTOHS((USHORT)localPort);

		pgn.dest.addr6.sin6_family = AF_INET6;
		pgn.dest.addr6.sin6_addr = *remoteAddr;
		pgn.dest.addr6.sin6_port = (USHORT)NTOHS((USHORT)remotePort);
											...
											VOID* packet, const FWPS_FILTER0* filter, UINT64 flowContext, FWPS_CLASSIFY_OUT0* classifyOut)
{
	...
	if(!action) {
		...
		pgn.source.addr6.sin6_family = AF_INET6;
		pgn.source.addr6.sin6_addr = *remoteAddr;
		pgn.source.addr6.sin6_port = (USHORT)NTOHS((USHORT)remotePort);

		pgn.dest.addr6.sin6_family = AF_INET6;
		pgn.dest.addr6.sin6_addr = *localAddr;
		pgn.dest.addr6.sin6_port = (USHORT)NTOHS((USHORT)localPort);
