...
/*
	this is a basic wrapper for WFP sessions on Vista, to keep error checking in
	one place.
*/
class wfp_session {
public:
	...
	void open(const FWPM_SESSION0 &session) {
		if(m_session) throw std::runtime_error("session already open");
...
private:
	// noncopyable.
	wfp_session(const wfp_session&);
	void operator=(const wfp_session&);
