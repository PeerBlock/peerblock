...
static BOOLEAN APIENTRY rng_win32_init(void *buf, ULONG len);

static BOOLEAN (APIENTRY *g_RtlGenRandom)(void*, ULONG) = rng_win32_init;

static BOOLEAN APIENTRY rng_win32_legacy(void *buf, ULONG len) {
	HCRYPTPROV hProv = 0;
	BOOL ret;
   ...
   if (!CryptAcquireContext(&hProv, NULL, MS_DEF_PROV, PROV_RSA_FULL, 
	...
	ret = CryptGenRandom(hProv, len, buf);
...
static BOOLEAN APIENTRY rng_win32_init(void *buf, ULONG len) {
	...
	HMODULE mod = GetModuleHandle(L"advapi32.dll");
	if(mod) {
		func = GetProcAddress(mod, "SystemFunction036");
	}

	g_RtlGenRandom = func ? func : rng_win32_legacy;

	return g_RtlGenRandom(buf, len);
}

static unsigned long rng_win32(unsigned char *buf, unsigned long len, void (*callback)(void)) {
	return g_RtlGenRandom(buf, len) ? len : 0;
}
