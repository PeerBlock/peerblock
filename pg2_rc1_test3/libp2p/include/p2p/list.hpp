...
namespace p2p {
	...
	public:
		...
		const_iterator begin() const { return this->_ranges.begin(); }
		const_iterator end() const { return this->_ranges.end(); }

		size_type size() const { return this->_ranges.size(); }
		void clear() { this->_ranges.clear(); }
		...
		list() {}
		list(const path_type &file, file_type type=file_auto) { this->load(file, type); }
		list(istream_type &stream, file_type type=file_auto) { this->load(stream, type); }
	};
}
