...
// ~PgFilter will block for up to around sleep_ms milliseconds.
static const DWORD loop_count=20;
static const DWORD sleep_ms=25;

pgfilter::pgfilter() : m_thread(NULL),m_blockhttp(true) {
	//this->setblock(true);
}
...
DWORD WINAPI pgfilter::thread_thunk(void *arg) {
	reinterpret_cast<pgfilter*>(arg)->thread_func();
	return 0;
}

const pgfilter::range *pgfilter::inranges(const range *ranges, int count, unsigned int ip) {
...
void pgfilter::setranges(const p2p::list &ranges, bool block) {
...
void pgfilter::setactionfunc(const action_function &func) {
	mutex::scoped_lock lock(m_lock);
	m_onaction = func;
}

void pgfilter::setblock(bool block) {
...
void pgfilter::setblockhttp(bool block) {
	m_blockhttp = block;
}
