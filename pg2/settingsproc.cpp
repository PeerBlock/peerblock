		...
		switch(msg) {
			case WM_COMMAND:
				switch(LOWORD(wParam)) {
					case IDC_STARTWITHWINDOWS: {
						...
						if(RegOpenKeyEx(HKEY_CURRENT_USER, _T("Software\\Microsoft\\Windows\\CurrentVersion\\Run"), 0, KEY_WRITE, &key)==ERROR_SUCCESS) {
							if(IsDlgButtonChecked(hwnd, IDC_STARTWITHWINDOWS)==BST_CHECKED) {
								...
								if(len) RegSetValueEx(key, _T("PeerGuardian"), 0, REG_SZ, (LPBYTE)dir, (len+1)*sizeof(TCHAR));
							}
							else RegDeleteValue(key, _T("PeerGuardian"));
					...
					case IDC_CHECKPG:
					case IDC_CHECKLISTS: {
						const bool ba=g_config.UpdatePeerGuardian=(IsDlgButtonChecked(hwnd, IDC_CHECKPG)==BST_CHECKED);
			...
			case WM_INITDIALOG: {
				...
				if(g_config.UpdatePeerGuardian || g_config.UpdateLists) {
					if(g_config.UpdatePeerGuardian) CheckDlgButton(hwnd, IDC_CHECKPG, BST_CHECKED);
				...
				if(RegOpenKeyEx(HKEY_CURRENT_USER, _T("Software\\Microsoft\\Windows\\CurrentVersion\\Run"), 0, KEY_READ, &key)==ERROR_SUCCESS) {
					...
					if(RegQueryValueEx(key, _T("PeerGuardian"), NULL, &type, (LPBYTE)data, &datalen)==ERROR_SUCCESS && type==REG_SZ && !_tcscmp(data, dir))
